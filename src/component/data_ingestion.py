...
class DataIngestion:
    ...
    def __init__(self, data_ingestion_config: DataIngestionConfig, n_retry: int = 5):
        ...
        n_month_interval: n month data will be download
        """
        try:
    ...
    def download_files(self, n_day_interval_url: int = None):
        ...
        ================================================================================
        return: List of DownloadUrl = namedtuple("DownloadUrl", ["url", "file_path", "n_retry"])
        """
        try:
        ...
        try:
            ...
            try:
                ...
                with open(download_url.file_path, "w") as file_obj:
                    finance_complain_data = list(map(lambda x: x["_source"],
                                                     filter(lambda x: "_source" in x.keys(),
                                                            json.loads(data.content)
                                                            )
                                                        )
                                                    )
                    json.dump(finance_complain_data, file_obj)
            except Exception as e:
    ...
    def retry_download_data(self, data, download_url: DownloadUrl):
        """
        This function help to avoid failure as it helps to download failed file again
        ...
        try:
            ...
            # To handle throatling requestion and can be solve if we wait for some second
    ...
    def convert_files_to_parquet(self,) -> str:
        ...
        json_data_dir    : download json file directory
        ...
        try:
            ...
            for file_name in os.listdir(json_data_dir):
                ...
                logging.info(f"Converting {json_file_path} into parquet format at {file_path}")
    ...
    def initiate_data_ingestion(self) -> DataIngestionArtifact:
        ...
        try:
            logging.info("Started downloading json file")
            ...
            if os.path.exists(self.data_ingestion_config.download_dir):
                logging.info("Converting and combining json into a parquest file")
